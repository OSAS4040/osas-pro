...
class WorkOrderOrderNumberSequenceTest extends TestCase
{
    ...
    public function test_scoped_sequence_per_customer_and_org_unit_uses_sector_prefix(): void
    {
        $company = $this->createCompany();
        $branch = $this->createBranch($company);
        ...
        $user = $this->createUser($company, $branch, 'owner', ['org_unit_id' => $division->id]);
        $customer = Customer::create([
            'uuid' => Str::uuid(),
            'company_id' => $company->id,
            'created_by_user_id' => $user->id,
        ...
        $payload = [
            ...
            'vehicle_id' => $vehicle->id,
            'items' => [
                ['item_type' => 'labor', 'name' => 'scope', 'quantity' => 1, 'unit_price' => 1, 'tax_rate' => 0],
            ],
        ];

        $o1 = $service->create($payload, $company->id, $branch->id, $user->id);
        $o2 = $service->create($payload, $company->id, $branch->id, $user->id);

        $this->assertSame(sprintf('WO-%d-OPS-C%d-%06d', $company->id, $customer->id, 1), $o1->order_number);
        $this->assertSame(sprintf('WO-%d-OPS-C%d-%06d', $company->id, $customer->id, 2), $o2->order_number);
    }
