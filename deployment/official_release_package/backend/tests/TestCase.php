...
abstract class TestCase extends BaseTestCase
{
    ...
    /**
     * WorkOrderService requires at least one persisted after_service_images row before Completed;
     * technician_notes / mileage_out may be supplied here when tests call transition(Completed) without meta.
     *
     * @param  array<string, mixed>  $persisted
     */
    protected function prepareWorkOrderForCompletedTransition(WorkOrder $order, array $persisted = []): void
    {
        $existing = is_array($order->after_service_images) ? $order->after_service_images : [];
        $images = count($existing) > 0
            ? $existing
            : ['work-orders/'.$order->id.'/after/'.Str::lower(Str::random(10)).'.jpg'];

        $order->forceFill(array_merge(['after_service_images' => $images], $persisted))->save();
        $order->refresh();
    }
}
