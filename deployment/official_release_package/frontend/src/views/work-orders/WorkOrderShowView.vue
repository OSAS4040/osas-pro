<template>
  <div class="space-y-6" dir="rtl" :class="{ '!space-y-3': staffUi.compactMode }">
    ...
    <template v-else-if="order">
      ...
      <!-- البنود — عرض مع أسعار ومصدر التسعير -->
        ...
        <table class="w-full" :class="staffUi.compactMode ? 'text-xs' : 'text-sm'">
          <thead class="bg-gray-50 text-xs text-gray-500">
            <tr>
              ...
              <th class="px-4 py-3 text-right">مصدر السعر</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            <tr v-for="item in order.items" :key="item.id">
              ...
              <td class="px-4 py-3 text-right tabular-nums">{{ fmtMoney(item.unit_price) }}</td>
              <td class="px-4 py-3 text-right tabular-nums font-medium">{{ fmtMoney(item.total) }}</td>
              <td class="px-4 py-3 text-right text-gray-500 text-[11px] leading-snug max-w-[9rem]">
                {{ pricingSourceLabelAr(item.pricing_source) || '—' }}
              </td>
            </tr>
      ...
      <!-- تعديل البنود — مسودة أو بانتظار اعتماد المدير فقط -->
      <div
        v-if="canEditItems"
        class="bg-white rounded-xl border border-gray-200 space-y-3"
        :class="staffUi.compactMode ? 'p-3' : 'p-5'"
      >
        <h3 class="font-medium text-gray-800" :class="{ '!text-sm': staffUi.compactMode }">تعديل البنود والتسعير</h3>
        <p class="text-xs text-gray-500 leading-relaxed">
          البنود المربوطة بخدمة من الكتالوج تُسعّر من الخادم حسب العقد أو سياسة التسعير عند الحفظ، ولا يُعتمد تعديل السعر يدوياً لهذه البنود.
        </p>
        <WorkOrderLinesEditor
          v-model="editItems"
          :customer-id="String(order.customer_id ?? '')"
          :vehicle-id="String(order.vehicle_id ?? '')"
          :services="services"
        />
        ...
        <div class="flex justify-end">
          <button
            ...
            :disabled="itemsSaving || !editItems.length"
            @click="saveWorkOrderItems"
          >
            {{ itemsSaving ? 'جارٍ الحفظ…' : 'حفظ البنود' }}
          </button>
        </div>
      </div>
...
import {
  ...
  catalogLineFromApiItem,
  canSubmitCatalogLines,
  buildItemsApiPayload,
  pricingSourceLabelAr,
...
const editItems = ref<CatalogLineItem[]>([])
const itemsSaving = ref(false)
const itemsError = ref('')

const canEditItems = computed(() => {
  if (!order.value) return false
  if (!auth.hasPermission('work_orders.update')) return false
  const st = order.value.status
  return st === 'draft' || st === 'pending_manager_approval'
})
...
function syncEditItemsFromOrder() {
  if (!order.value?.items) {
    editItems.value = []
    return
  }
  editItems.value = order.value.items.map((it: Record<string, unknown>) => catalogLineFromApiItem(it))
}

async function loadServicesIfNeeded() {
  if (!canEditItems.value || services.value.length) return
  try {
    ...
    /* يبقى التعديل ممكناً للبنود اليدوية */
  }
}

async function saveWorkOrderItems() {
  if (!order.value || !canEditItems.value) return
  const cid = String(order.value.customer_id ?? '')
  const vid = String(order.value.vehicle_id ?? '')
  if (!canSubmitCatalogLines(editItems.value, cid, vid)) {
    itemsError.value =
      'تأكد من البنود: للبند المربوط بخدمة يجب أن يبقى السعر المعتمد من الخادم صالحاً (جرّب إعادة اختيار الخدمة إن لزم). للبند اليدوي أدخل الاسم والسعر.'
    return
  }
  ...
  try {
    const { data } = await apiClient.put(
      `/work-orders/${id}`,
      {
        ...
        items: buildItemsApiPayload(editItems.value),
      },
      { skipGlobalErrorToast: true },
    )
    order.value = data.data
    syncEditItemsFromOrder()
    toast.success('تم الحفظ', 'تم تحديث بنود أمر العمل.')
  } catch (e: unknown) {
    itemsError.value = summarizeAxiosError(e)
  } finally {
