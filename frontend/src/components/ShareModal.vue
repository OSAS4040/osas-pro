<template>
  ...
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="show" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4"
        ...
        <div class="relative w-full max-w-sm bg-white dark:bg-slate-800 rounded-2xl shadow-2xl overflow-hidden"
          @click.stop>

          <!-- Header -->
          <div class="px-5 py-4 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between">
            <div>
              <h3 class="font-semibold text-gray-900 dark:text-slate-100 text-sm">مشاركة {{ label }}</h3>
              <p class="text-xs text-gray-400 mt-0.5 truncate max-w-[220px]">{{ title }}</p>
            </div>
            <button @click="show = false"
              class="w-8 h-8 rounded-full bg-gray-100 dark:bg-slate-700 flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-slate-200 transition-colors">
          ...
          <div class="px-5 pt-4 pb-2 flex flex-col items-center gap-3">
            <div class="bg-white rounded-2xl p-3 shadow-md border border-gray-100" ref="qrWrap">
              <!-- Real QR via canvas -->
          ...
          <div class="px-5 pb-5 space-y-3">
            <div class="grid grid-cols-3 gap-2">
              ...
              <button @click="shareWhatsApp"
                class="flex flex-col items-center gap-1.5 p-3 rounded-xl bg-green-50 dark:bg-green-900/20 hover:bg-green-100 dark:hover:bg-green-900/40 border border-green-100 dark:border-green-800 transition-colors group">
              ...
              <button @click="openEmailModal"
                class="flex flex-col items-center gap-1.5 p-3 rounded-xl bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-900/40 border border-blue-100 dark:border-blue-800 transition-colors">
              ...
              <button @click="shareSMS"
                class="flex flex-col items-center gap-1.5 p-3 rounded-xl bg-purple-50 dark:bg-purple-900/20 hover:bg-purple-100 dark:hover:bg-purple-900/40 border border-purple-100 dark:border-purple-800 transition-colors">
              ...
              <button @click="copyLink"
                class="flex flex-col items-center gap-1.5 p-3 rounded-xl bg-gray-50 dark:bg-slate-700 hover:bg-gray-100 dark:hover:bg-slate-600 border border-gray-100 dark:border-slate-600 transition-colors">
              ...
              <button @click="downloadQR"
                class="flex flex-col items-center gap-1.5 p-3 rounded-xl bg-orange-50 dark:bg-orange-900/20 hover:bg-orange-100 dark:hover:bg-orange-900/40 border border-orange-100 dark:border-orange-800 transition-colors">
              ...
              <button v-if="canNativeShare" @click="nativeShare"
                class="flex flex-col items-center gap-1.5 p-3 rounded-xl bg-teal-50 dark:bg-teal-900/20 hover:bg-teal-100 dark:hover:bg-teal-900/40 border border-teal-100 dark:border-teal-800 transition-colors">
            ...
            <div class="border border-gray-100 dark:border-slate-700 rounded-xl p-3">
              ...
              <div class="flex gap-2">
                ...
                <button @click="shareWhatsAppCustom"
  ...
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="emailModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4" @click.self="emailModal = false">
        <div class="absolute inset-0 bg-black/60 backdrop-blur-sm" @click="emailModal = false" />
        <div class="relative w-full max-w-sm bg-white dark:bg-slate-800 rounded-2xl shadow-2xl p-6 space-y-4" @click.stop>
          <div class="flex items-center justify-between">
            <h4 class="font-semibold text-gray-900 dark:text-slate-100 text-sm">إرسال بالبريد الإلكتروني</h4>
            <button @click="emailModal = false" class="text-gray-400 hover:text-gray-600">
              <XMarkIcon class="w-4 h-4" />
            </button>
          </div>
          <div>
          ...
          <div>
            ...
            <textarea v-model="emailBody" rows="3"
              class="w-full px-3 py-2.5 border border-gray-200 dark:border-slate-600 rounded-xl text-sm dark:bg-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"></textarea>
          </div>
          <div class="flex gap-2">
            <button @click="sendEmail" :disabled="sendingEmail || !emailTo"
            ...
            <button @click="openMailtoFallback"
              class="px-4 py-2.5 border border-gray-200 dark:border-slate-600 rounded-xl text-sm text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
              فتح في Mail
            </button>
          </div>
        </div>
