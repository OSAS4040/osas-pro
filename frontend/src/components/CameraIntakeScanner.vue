...
import { ref, onUnmounted, nextTick, toRef } from 'vue'
...
const props = withDefaults(
  defineProps<{
    /** مشغّل منصّة: البحث في سياق مزوّد (شريك تنفيذ) */
    onBehalfCompanyId?: number | null
  }>(),
  { onBehalfCompanyId: null },
)
const onBehalfCompanyIdRef = toRef(props, 'onBehalfCompanyId')
...
async function processPlateImage(canvas: HTMLCanvasElement) {
  ...
  try {
    ...
    if (onBehalfCompanyIdRef.value != null && onBehalfCompanyIdRef.value > 0) {
      body.on_behalf_company_id = onBehalfCompanyIdRef.value
    }
    const { data: laravelBody } = await apiClient.post('/work-orders/intake-lookup-camera', body)
