  ...
  <div dir="rtl">
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex gap-2">
        <button @click="showNewArticle = true"
    ...
    <div class="relative mb-6">
      ...
      <input v-model="search" @input="fetchArticles" placeholder="ابحث في قاعدة المعرفة..."
        class="w-full border border-gray-300 dark:border-gray-600 rounded-xl pr-10 pl-4 py-3 text-sm dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none" />
    </div>

    <!-- Categories -->
    <div class="flex gap-2 flex-wrap mb-6">
      <button @click="selectedCat = ''; fetchArticles()"
        :class="!selectedCat ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700'"
    ...
    <div v-if="!search && !selectedCat" class="mb-6">
      ...
      <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div v-for="a in featured" :key="a.id" @click="openArticle(a)"
          class="bg-gradient-to-br from-indigo-50 to-blue-50 dark:from-indigo-900/30 dark:to-blue-900/30 border border-indigo-100 dark:border-indigo-800 rounded-xl p-4 cursor-pointer hover:shadow-md transition-all">
    ...
    <div class="space-y-3">
      <div v-for="a in articles" :key="a.id" @click="openArticle(a)"
        class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 cursor-pointer hover:border-indigo-300 dark:hover:border-indigo-600 hover:shadow-sm transition-all">
      ...
      <div v-if="selectedArticle" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" dir="rtl">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col">
          ...
          <div class="p-4 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
            ...
            <div class="flex gap-2">
              <button @click="vote(selectedArticle.id, true)" class="px-4 py-2 bg-green-100 hover:bg-green-200 text-green-700 rounded-lg text-sm font-medium transition-all">
                👍 نعم
              </button>
              <button @click="vote(selectedArticle.id, false)" class="px-4 py-2 bg-red-100 hover:bg-red-200 text-red-700 rounded-lg text-sm font-medium transition-all">
                👎 لا
              </button>
            </div>
          </div>
    ...
    <Teleport to="body">
      <div v-if="showNewArticle" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" dir="rtl" @click.self="showNewArticle = false">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-xl">
          ...
          <form @submit.prevent="createArticle" class="p-5 space-y-4">
            ...
            <div class="flex items-center gap-4">
              <label class="flex items-center gap-2 text-sm">
                <input type="checkbox" v-model="newArticle.is_featured" class="rounded" /> مميز
              </label>
            ...
            <div class="flex justify-end gap-2">
              <button type="button" @click="showNewArticle = false" class="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg">إلغاء</button>
