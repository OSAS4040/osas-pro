<template>
  <div class="space-y-5" dir="rtl">
    ...
    <template v-else-if="vehicle">
      ...
      <!-- Tabs -->
      <div class="flex gap-1 bg-gray-100 dark:bg-slate-700/50 rounded-xl p-1 flex-wrap">
      ...
      <!-- ── Tab: معلومات ── -->
      <div v-if="activeTab === 'info'" class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm p-6">
        ...
        <div v-if="vehicle.customer" class="mt-5 pt-5 border-t border-gray-100 dark:border-slate-700">
          ...
          <RouterLink :to="`/customers`" class="font-semibold text-primary-600 dark:text-primary-400 hover:underline text-sm">
      ...
      <!-- ── Tab: تاريخ الخدمة ── -->
      <div v-else-if="activeTab === 'history'" class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
        ...
        <div v-else-if="workOrders.length" class="divide-y divide-gray-100 dark:divide-slate-700">
          <div v-for="wo in workOrders" :key="wo.id"
            ...
            <span class="px-2 py-0.5 rounded-full text-xs font-medium capitalize"
              :class="woStatusClass(wo.status)">{{ wo.status }}</span>
          </div>
        </div>
      ...
      <!-- ── Tab: المستندات ── -->
      <div v-else-if="activeTab === 'docs'" class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm p-6">
      ...
      <!-- ── Tab: الإعدادات ── -->
      <div v-else-if="activeTab === 'settings'" class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm p-6">
      ...
      <!-- ── Tab: الباركود ── -->
      <div v-else-if="activeTab === 'barcode'" class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm p-6 text-center">
        <h2 class="font-bold text-gray-800 dark:text-white mb-5 flex items-center justify-center gap-2">
          <QrCodeIcon class="w-5 h-5 text-teal-500" />
          الباركود
        </h2>
        <div class="inline-flex flex-col items-center gap-4">
          <div class="w-40 h-40 bg-gray-100 dark:bg-slate-700 rounded-xl flex items-center justify-center border-2 border-dashed border-gray-300 dark:border-slate-600">
            <QrCodeIcon class="w-20 h-20 text-gray-300 dark:text-slate-500" />
          </div>
          <p class="text-sm text-gray-600 dark:text-slate-400">رقم اللوحة: <strong class="font-mono text-gray-900 dark:text-white">{{ vehicle.plate_number }}</strong></p>
          <RouterLink :to="`/vehicles/${vehicle.id}/card`"
    ...
    <!-- Error / not found -->
    <div v-else class="text-center py-16 text-gray-400 dark:text-slate-500">
      <TruckIcon class="w-12 h-12 mx-auto mb-3 opacity-30" />
      <p>تعذّر تحميل بيانات المركبة</p>
    </div>
  </div>
</template>
...
function woStatusClass(s: string) {
  ...
  }[s] ?? 'bg-gray-100 text-gray-600 dark:bg-slate-700 dark:text-slate-300'
}
