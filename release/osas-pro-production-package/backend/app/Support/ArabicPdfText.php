 ...
 *
 * المعتمد: **النص المنطقي UTF-8 كما هو** داخل span بـ `dir="rtl"` مع `unicode-bidi: isolate` في CSS القالب
 * (تجنّب `embed` + `inline-block` على Dompdf 3 — غالباً يسبّب انعكاساً بصرياً للكلمات العربية).
...
final class ArabicPdfText
{
    ...
    public static function asDompdfHtml(?string $text): HtmlString
    {
        ...
        if (! preg_match('/\p{Arabic}/u', $text)) {
            return new HtmlString(
                '<span class="dompdf-ar-ascii" dir="ltr">'.e($text).'</span>'
            );
        }

        return new HtmlString(
            '<span class="dompdf-ar-shape" dir="rtl" lang="ar">'.e($text).'</span>'
        );
    }
