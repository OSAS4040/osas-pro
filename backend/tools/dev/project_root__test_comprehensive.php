...
test('Company model', fn () => Company::count() >= 0);
test('Plan model exists', fn () => Plan::count() >= 0);
test('Professional plan has fleet feature', function () {
    $plan = Plan::where('slug', 'professional')->first();
    if (! $plan) {
        throw new Exception('professional plan not found');
    }
    $features = $plan->features;
    if (is_array($features) && isset($features['fleet'])) {
        return $features['fleet'] === true;
    }
    if (is_array($features) && in_array('fleet', $features)) {
        return true;
    }
...
test('Demo company subscription', function () {
    ...
    if (! $company) {
        throw new Exception('demo company not found');
    }
...
test('User model', fn () => User::count() >= 0);
...
test('fleet.contact@demo.sa user exists', function () {
    $user = User::where('email', 'fleet.contact@demo.sa')->first();
    if (! $user) {
        throw new Exception('fleet contact not found');
    }
...
// ── 3. Invoice System ──
test('Invoice model', fn () => Invoice::count() >= 0);
test('Invoice with items relationship', function () {
    $inv = Invoice::with('items')->first();

    return $inv !== null || true; // pass if no invoices yet
});

// ── 4. Vehicle System ──
test('Vehicle model', fn () => Vehicle::count() >= 0);

// ── 5. Fleet / Wallet ──
test('Fleet wallet system (Wallet + CustomerWallet)', function () {
    $hasWallet = class_exists(Wallet::class);
    $hasCW = class_exists(CustomerWallet::class);
    if (! $hasWallet || ! $hasCW) {
        throw new Exception('Wallet models missing');
    }

    return true;
});
test('Wallet model', function () {
    $exists = class_exists(Wallet::class);
    if (! $exists) {
        throw new Exception('Wallet model missing');
    }
...
// ── 6. Work Orders ──
test('WorkOrder model', fn () => WorkOrder::count() >= 0);

// ── 7. Bays / BookingSlots ──
test('Bay model', function () {
    $exists = class_exists(Bay::class);
    if (! $exists) {
        throw new Exception('Bay model missing');
    }
...
test('Employee model', function () {
    $exists = class_exists(Employee::class);
    if (! $exists) {
        throw new Exception('Employee model missing');
    }
...
test('API routes loaded', function () {
    ...
    if ($routes->count() < 5) {
        throw new Exception('Too few API routes: '.$routes->count());
    }
...
test('KPI query < 2000ms', function () {
    ...
    if ($ms > 2000) {
        throw new Exception("Queries took {$ms}ms");
    }
...
test('Invoice hash field exists', function () {
    ...
    if (! in_array('invoice_hash', $cols)) {
        throw new Exception('invoice_hash column missing from invoices');
    }
...
test('JournalEntry model or ledger_entries table', function () {
    ...
    if (empty($tables)) {
        throw new Exception('No ledger/journal table found');
    }
