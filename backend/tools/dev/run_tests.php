...
test('مخزون', 'التحقق من رصيد المخزون', function () use ($inventoryService, $user, $productId) {
    ...
    if ($level['quantity'] < 200) {
        throw new Exception("الكمية غير صحيحة: {$level['quantity']}");
    }
...
test('مخزون', 'التحقق من الكمية بعد الخصم (200 - 30 = 170)', function () use ($inventoryService, $user, $productId) {
    ...
    if (abs($balance - 170) > 0.01) {
        throw new Exception("المتوقع 170، الفعلي: {$balance}");
    }
...
test('مخزون', 'عكس الحركة (reverseMovement)', function () use ($inventoryService, $user, $productId, $movement2) {
    if (! $movement2) {
        throw new Exception('لا توجد حركة للعكس');
    }
    $reversal = $inventoryService->reverseMovement($movement2, $user->id, app('trace_id'));
    $balance = $inventoryService->getBalance($user->company_id, $user->branch_id, $productId);
    if (abs($balance - 200) > 0.01) {
        throw new Exception("بعد العكس المتوقع 200، الفعلي: {$balance}");
    }
...
test('عملاء', 'قراءة بيانات العميل', function () use ($customerId) {
    if (! $customerId) {
        throw new Exception('لا يوجد عميل');
    }
    $c = DB::table('customers')->where('id', $customerId)->first();
    echo "   → الاسم: {$c->name} | الهاتف: {$c->phone}\n";
});

test('عملاء', 'تعديل بيانات العميل', function () use ($customerId) {
    if (! $customerId) {
        throw new Exception('لا يوجد عميل');
    }
    ...
    if ($name !== 'أحمد محمد المحدّث') {
        throw new Exception('التعديل فشل');
    }
...
test('عملاء', 'البحث في العملاء', function () use ($user) {
    ...
    if ($count < 1) {
        throw new Exception('لم يُعثر على عميل');
    }
...
$supplierId = test('موردون', 'إنشاء مورد جديد', function () use ($user, $rand, $supplierCols) {
    ...
    if (in_array('uuid', $supplierCols)) {
        $data['uuid'] = Str::uuid();
    }
    if (in_array('phone', $supplierCols)) {
        $data['phone'] = "05-TEST-S{$rand}";
    }
    if (in_array('email', $supplierCols)) {
        $data['email'] = "sup{$rand}@example.com";
    }
...
test('موردون', 'قراءة بيانات المورد', function () use ($supplierId) {
    if (! $supplierId) {
        throw new Exception('لا يوجد مورد');
    }
    $s = DB::table('suppliers')->where('id', $supplierId)->first();
    echo "   → الاسم: {$s->name}\n";
});

test('موردون', 'تعديل بيانات المورد', function () use ($supplierId) {
    if (! $supplierId) {
        throw new Exception('لا يوجد مورد');
    }
...
test('فواتير', 'التحقق من صحة سلسلة الهاش', function () use ($user) {
    ...
    foreach ($invoices as $inv) {
        ...
        if ($inv->invoice_hash !== $expected) {
            throw new Exception("الهاش غير صحيح: {$inv->invoice_number}");
        }
...
test('فواتير', 'فاتورة مرتبطة بعميل', function () use ($invoiceService, $user, $productId, $product, $customerId) {
    ...
    if ($invoice->customer_id !== $customerId) {
        throw new Exception('العميل لم يُربط');
    }
