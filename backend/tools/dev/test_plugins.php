...
if ($r['status'] === 200 && $count > 0) {
    echo "✅ GET /plugins [$count plugins]\n";
    $passed++;
} else {
    echo "❌ GET /plugins [{$r['status']}]\n";
    $failed++;
}
...
if (in_array($r['status'], [200, 201])) {
    ...
    echo "❌ Install plugin [{$r['status']}]: ".json_encode($r['body'])."\n";
    $failed++;
}

$r = api('POST', "$base/plugins/ai_advanced_diagnostics/execute", ['context' => ['make' => 'Toyota', 'model' => 'Camry', 'symptoms' => ['noise']]], $token);
if ($r['status'] === 200 && isset($r['body']['data']['diagnostics'])) {
    echo "✅ Execute plugin AI diagnostics\n";
    $passed++;
} else {
    echo "❌ Execute plugin [{$r['status']}]\n";
    $failed++;
}

$r = api('GET', "$base/plugins/tenant", null, $token);
if ($r['status'] === 200) {
    echo '✅ GET /plugins/tenant [count='.count($r['body']['data'] ?? [])."]\n";
    $passed++;
} else {
    echo "❌ GET /plugins/tenant [{$r['status']}]\n";
    $failed++;
}
...
if (in_array($r['status'], [200, 204])) {
    ...
    echo "❌ Uninstall plugin [{$r['status']}]\n";
    $failed++;
}
