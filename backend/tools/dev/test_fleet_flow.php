...
function ikey(string $p = 'k'): string
{
    return "$p-".time().'-'.bin2hex(random_bytes(4));
}
...
if ($r['code'] === 200 && ! empty($r['body']['token'])) {
    ...
    fail('تسجيل دخول fleet_contact', "HTTP {$r['code']}");
    exit(1);
}
...
if ($r['code'] === 200 && ! empty($r['body']['token'])) {
    ...
    fail('تسجيل دخول fleet_manager', "HTTP {$r['code']}");
    exit(1);
}
...
if ($r['code'] === 200 && ! empty($r['body']['token'])) {
    ...
    fail('تسجيل دخول الورشة', "HTTP {$r['code']}");
    exit(1);
}
...
if ($woWallet) {
    pass("  رقم الطلب: #{$r['body']['data']['order_number']}");
}
...
if ($woCreditId) {
    ...
    if ($approved) {
        pass('  credit_authorized = true ✓');
    } else {
        fail('  credit_authorized يجب أن يكون true');
    }
...
if ($r['code'] === 201) {
    ...
    if ($rejStatus === 'rejected') {
        ...
        fail('  approval_status يجب أن يكون rejected', "actual=$rejStatus");
    }
} else {
