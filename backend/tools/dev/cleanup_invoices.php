...
{
    $ch = curl_init($u);
    curl_setopt_array($ch, [CURLOPT_RETURNTRANSFER => true, CURLOPT_CUSTOMREQUEST => strtoupper($m), CURLOPT_HTTPHEADER => array_merge(['Content-Type: application/json', 'Accept: application/json'], $h), CURLOPT_TIMEOUT => 15]);
    if ($b) {
...
$conflictingInvoices = DB::table('invoices')
    ->where('company_id', 1)
    ->whereIn('invoice_number', ['INV-1-000001', 'INV-1-000002', 'INV-1-000003', 'INV-1-000004', 'INV-1-000005'])
    ->pluck('id');
$pDel = DB::table('payments')->whereIn('invoice_id', $conflictingInvoices)->delete();
...
$maxCounter = DB::table('invoices')->where('company_id', 1)->max('invoice_counter') ?? 0;
