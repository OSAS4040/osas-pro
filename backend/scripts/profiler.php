...
if (empty($slow)) {
    ...
    foreach ($slow as $r) {
        $reason = '';
        if (str_contains($r['label'], 'Redis')) {
            $reason = '→ network overhead / Laravel serialization';
        } elseif (str_contains($r['label'], 'JOIN')) {
            $reason = '→ missing index on FK column';
        } elseif (str_contains($r['label'], 'with')) {
            $reason = '→ N+1 / missing eager load index';
        } else {
            $reason = '→ review query plan';
        }
