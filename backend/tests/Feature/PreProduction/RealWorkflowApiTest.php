...
class RealWorkflowApiTest extends TestCase
{
    ...
    public function test_end_to_end_operational_chain_and_data_integrity(): void
    {
        ...
        $splitInvoices = Invoice::withoutGlobalScopes()
            ...
            ->where('source_id', $woId)
            ->get();
        $this->assertCount(2, $splitInvoices, 'تقسيم الفوترة: فاتورة مزوّد→منصة + فاتورة منصة→عميل لنفس أمر العمل');
        $this->assertSame(1, $splitInvoices->where('billing_flow_type', 'provider_to_platform')->count());
        $this->assertSame(1, $splitInvoices->where('billing_flow_type', 'platform_to_customer')->count());
